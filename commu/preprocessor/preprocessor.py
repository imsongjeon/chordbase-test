...
class Preprocessor:
        ...
        for sample_info_idx, sample_info in enumerate(copied_sample_infos_chunk):
            ...
            if sample_info.get("augmented", False):
                ...
                try:
                    ...
                    np.save(
                        os.path.join(output_dir, f"target_{sample_info_idx}"), np.array(encoding_output.event_sequence, dtype=object),  allow_pickle=True
                    )
                except AttributeError as e:
                    print(f"Error saving target file for sample {sample_info_idx}: {e}")
                    continue
    ...
    def concat_npy(source_dir: Union[str, Path]) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        ...
        def _concat(_npy_list: List[str]) -> List[np.ndarray]:
            return [np.load(_p, allow_pickle=True) for _p in tqdm.tqdm(_npy_list)]
