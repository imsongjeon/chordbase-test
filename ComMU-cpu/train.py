...
def train():
    ...
    for batch, (data, target, reset_mems, batch_token_num) in tqdm.tqdm(enumerate(
            train_real_iter
    )):
        ...
        if train_step % cfg.TRAIN.log_interval == 0:
            ...
            log_grad_norm /= cfg.TRAIN.log_interval * 4
        ...
        if train_step % cfg.TRAIN.eval_interval == 0:
            ...
            val_total_nll_pt = torch.tensor(val_total_nll / 10000.0, dtype=torch.float32).to(device)
            ...
            if not best_val_nll or val_nll < best_val_nll:
                ...
                def calculate_test_nll_during_training(test_iter):
                    ...
                    test_total_nll_pt = torch.tensor(test_total_nll / 10000.0, dtype=torch.float32).to(device)
